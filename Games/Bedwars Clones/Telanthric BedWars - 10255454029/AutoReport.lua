...
function getDescription(PlayerUID: number | string)
...
function Report(Player, NotifyMethod)
	...
	if Player == game.Players.LocalPlayer or includes(reportedPlrs, Player) then
		return
	end

	local URL =
		"https://docs.google.com/forms/d/e/1FAIpQLSfzOZz9FddF3TpbG1BEtuKlP_mVlH00D4JIYuHLEnBE2XwuaA/formResponse"
	local formData = {
	...
	if not response.Success then
		...
		for hName, hValue in response do
			if hName == "Body" then
			end
