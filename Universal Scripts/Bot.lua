...
if isLegacy then
    local ChatEvents: Folder? = ReplicatedStorage
...
local function format(str, argsTable)
    if not str then error("Unable to format - no string provided.") end
    if type(str) ~= "string" then error("Unable to format - expected \"string\", got \""..type(str).."\" instead for argument #1.") end
    if not argsTable then error("Unable to format - no argument(s) table provided") end
    if type(argsTable) ~= "table" then error("Unable to format - expected \"table\", got \""..type(argsTable).."\" for argument #2.") end
    for name, value in argsTable do
        if not str:find("{"..name.."}") then continue end
...
local function req(opt)
	local fn = if Fluxus then Fluxus.request
		elseif request then request
		elseif syn then syn.request else nil
	if fn then
		return fn(opt)
	end
end
...
function connections:getConnections()
	...
	for _, connHolder: {{RBXScriptConnection}} in connections do
		if type(connHolder) ~= "table" then continue end
...
local function GetPlayerByName(name: string, caseInsensitive: boolean?, requestingPlayer: Player?): Player?
	...
	for _, player in Players:GetPlayers() do
		if (caseInsensitive and player.Name:lower() == name:lower()) or (not caseInsensitive and player.Name == name) then
			return player
        elseif (caseInsensitive and player.DisplayName:lower() == name:lower())
                or (not caseInsensitive and player.DisplayName == name) then
            return player
		end
	end
	return nil
...
local function Chat(message: string, broadcast: boolean, target: Player)
	if broadcast == nil or type(broadcast) ~= "boolean" then broadcast = true end
	if isLegacy then
		ChatEvent:FireServer(message, if broadcast then "All" else "To "..target)
	else
		local inputBarConfiguration: ChatInputBarConfiguration =
			TextChatService.ChatInputBarConfiguration
		task.spawn(inputBarConfiguration.TargetTextChannel.SendAsync,
...
local commands = {
    ...
    goto = {
        description = "Goes to the specified target player",
        callback = function(user, args)
        	local target = GetPlayerByName(args[1], true, user)
        	
        	if not target then
        		return Chat("Player \""..args[1].."\" doesn't appear to exist.")
        	end

            if not target.Character then
                return Chat("Player \""..args[1].."\" doesn't appear to have a character.")
            end
	...
	vclip = {
		...
		callback = function(user, args)
			...
			if not tonumber(args[1]) then
				Chat("@"..user.DisplayName.." I can't VClip,"..
			...
			Character:PivotTo(
				Character:GetPivot() + Vector3.new(
				0, tonumber(args[1]), 0
				)
			)
    ...
    -- },
    corn = {
        description = "Try some corn, or we will sacrfice your new born!!",
        callback = function()
            Chat("Try some corn, or we will sacrfice your new born! #cornborn 🌽")
        end
    },
	help = {
	...
	figlet = {
		description = "Turns some normal ASCII text "..
			"into a message that uses symbols.",
		callback = function(user, args)
			if not args[1] then
				Chat("@"..user.Name.." You didn't provide any arguments."..
					"Please try again.")
				return
			end
...
function commands.follow.callback(user, args)
    ...
    if not target then
        return Chat("Player you specified does not appear to exist, this "..
            "Only supports (actual) names (\"me\" is a exception, it'll refer to you.)", true)
    end
    local targetCharacter = target.Character or target.CharacterAdded:Wait()

    Character:FindFirstChildOfClass("Humanoid"):MoveTo(
        targetCharacter:GetPivot().Position
    )
end
...
local function handler(msg: string, player: Player)
    local commandName = msg:gsub(
        if not isLegacy
            then
            escapeHTML(config.prefix) -- TCS/TextChatService escapes some HTML characters,
            else config.prefix, "" -- and LCS/LegacyChatService doesn't escape any HTML characters.
        ):split(" ")[1]
    ...
    if not isValid then return Chat("Invalid command \""..commandName.."\"".."!") end
...
function registerPlayer(p: Player)
	if not isWhitelisted(p) then return nil end
    ...
    local function registerChattedEvent()
        return if isLegacy then
            registerChattedEventLCS()
            else
            registerChattedEventTCS()
    end
...
task.spawn(function()
	table.insert(connections.PlayerRemoving, Players.PlayerRemoving:Connect(function(p)
...
Chat(format(
    config.MessageFormats.StartMessage,
    {
